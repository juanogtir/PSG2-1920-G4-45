...
public class OwnerFormatter implements Formatter<Owner> {
	...
	@Autowired
	public OwnerFormatter(final OwnerRepository ownerRepository) {
	...
	public Owner parse(final String text, final Locale locale) throws ParseException {
		Iterable<Owner> findOwners = this.ownerRepository.findAll();
		for (Owner owner : findOwners) {
			if ((owner.getFirstName() + " " + owner.getLastName()).equals(text)) {
				return owner;
			}
		}
